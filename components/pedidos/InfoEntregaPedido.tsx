          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1 flex items-center gap-1">
            ...
            {esDelivery && !pedido.coordenadas && (
              <a
                ...
                className="text-blue-600 hover:underline inline-flex items-center gap-1 ml-1 font-bold text-[11px]"
                onClick={(e) => e.stopPropagation()}
              >
                <Search className="w-3 h-3" /> (Buscar en Maps)
              </a>
            )}
          </p>
      ...
      {esDelivery ? (
        pedido.coordenadas ? (
          <>
            ...
            >
              <MapPin className="w-3.5 h-3.5 text-chefsy-600 shrink-0" />
          ...
          >
            ...
            <span className="truncate max-w-[180px]">{resumen}</span>
            <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded ml-1 font-bold inline-flex items-center gap-0.5">
              <Search className="w-2.5 h-2.5" /> Maps
            </span>
          </a>
        )
      ) : (
