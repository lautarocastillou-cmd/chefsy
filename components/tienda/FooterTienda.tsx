    ...
    <footer className="mt-16 mb-24 w-full border-t border-white/10 pt-10 pb-8 px-4 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start text-left">
        ...
        <div className="md:col-span-7 space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-slate-400 text-xs font-semibold leading-relaxed">
              Nuestro local está ubicado en Rivadavia 195, antes de Almagro. Atendemos de Lunes a Sábado de 20:30hs a 01:00hs. ¡Domingo cerrado!
            </p>
            <a
              href={googleMapsUrl}
              target="_blank"
        ...
        <div className="md:col-span-5 flex flex-col items-start md:items-end gap-6 pt-1">
          {/* Redes Sociales */}
          {(() => {
            const igUrl = configuracion?.link_instagram || 'https://instagram.com/chefsy_fastfood_'
            const ttUrl = configuracion?.link_tiktok
            return (
              <div className="flex items-center gap-4">
                {igUrl && (
                  <a href={igUrl} target="_blank" rel="noreferrer" aria-label="Instagram" className="bg-white/5 hover:bg-chefsy-500 hover:text-black text-white p-2 rounded-full transition-all border border-white/10 hover:scale-105">
                    <Instagram size={18} />
                  </a>
                )}
                {ttUrl && (
                  <a href={ttUrl} target="_blank" rel="noreferrer" aria-label="TikTok" className="bg-white/5 hover:bg-chefsy-500 hover:text-black text-white p-2 rounded-full transition-all border border-white/10 hover:scale-105">
